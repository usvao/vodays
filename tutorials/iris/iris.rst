...
This use case is inspired by the notion of acquiring multi-wavelength data on
γ-ray sources, either to determine their counterparts or to determine the
emission mechanisms. From the standpoint of the Iris SED Tool, this use case
is intended to demonstrate how one could construct a Spectral Energy
Distribution (SED) using a mix of Virtual Observatory (VO) services and tools. 

Questions
^^^^^^^^^

* How do we now incorporate these data into a larger set of multi-wavelength 
  data to assess the SED of 3C273?
...
#. Select Load SED. A text box will open; provide a name for the new SED
   (e.g., 3C273). A new window should appear, with the ID specified.
#. In the Target Info area, enter the name 3C273 to resolve the source name
   into coordi- nates. Select Resolve and the coordinates should be filled in.
...
#. At the time of the writing of this document, the 2 Year Fermi data has not
   ...
   named “3c273_fermi.dat” containing the relevant Fermi data 
   (Abdo et al. 2011).
...
#. If you are not satisfied with the fit, you can edit the model. For example,
   ...
   chi2davatar (data points without error bars will be ignored during the fit);
